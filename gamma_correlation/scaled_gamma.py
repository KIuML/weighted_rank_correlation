...
def d_max(data, i, j, column):
    ...
    """
    return max(
        data[int(min(data[i, column], data[j, column])) - 1:int(max(data[i, column], data[j, column]) - 1), column + 2])
...
def d_sum(data, i, j, column):
    ...
    """
    return min(1,
               sum(data[int(min(data[i, column], data[j, column])) - 1:int(max(data[i, column], data[j, column]) - 1),
                   column + 2]))
    ...
    for i in range(len(data)):
        for z in range((i + 1), len(data)):
            ...
            if ((data[i, 0] == data[z, 0]) or (data[i, 1] == data[z, 1])):
                ...
                concordant += t_norm(R(data, i, z, 0, d), R(data, i, z, 1, d)) + t_norm(R(data, z, i, 0, d),
                                                                                        R(data, z, i, 1, d))
                discordant += t_norm(R(data, i, z, 0, d), R(data, z, i, 1, d)) + t_norm(R(data, z, i, 0, d),
                                                                                        R(data, i, z, 1, d))
